# Downloaded from https://github.com/MLWave/Kaggle_Rotten_Tomatoes/blob/master/plotfeatures.py
#
...
def show_most_informative_features(Y1W,Y1,Y2W,Y2,n=2):
	...
	for x,y,w in zip(X,Y1,Y1W):
		...
		text(x+0.4, max(Y1) + 0.2, '%s' % w, ha='center', va= 'bottom', rotation='vertical', size=7)
 
	for x,y,w in zip(X,Y2,Y2W):
		text(x+0.6, -0.2, '%.2f' % y, ha='center', va= 'center', rotation='vertical', size=7)
		text(x+0.4, min(Y2) - 0.4, '%s' % w, ha='center', va= 'top', rotation='vertical', size=7)
	xlim(-1,n), xticks([])
...
with open("data/varinfo.vw") as infile:
	d = {}
