...
class DataWrangling:
   ...
   def combine_hist_offer_transaggr_v1(self, infile='', infile1='',infile2='',outfile1='', outfile2=''):
       ...
       cat_to_gen= ['brand'] #['chain','category'] #,'company'] #,'brand']
       ...
       for i in xrange(1,3): #len(cat_to_gen)+1):
       ...
       print 'cat to combine ',cat_combinations
       #print cat_combinations_to_join
       data=pd.read_csv(infile)
       col_names=data.columns.values
       col_num= 12 #len(col_names)
       print col_num
       t_1=time.time()-t_i
       ...
       for i in xrange(len(cat_combinations)):
            ...
            if col_num>11:
               for j in xrange(col_num-11):
                   ...
                   new_names=[name+"_"+col_names[j+11]+"_"+str(i) for name in
                              ['len','ps_median','pq_median','pa_median']]
                   names=mergedata_train.columns.values
                   self.__rename_data(mergedata_train,names,new_names)
                   self.__rename_data(mergedata_test,names,new_names)
                   print mergedata_train.columns.values, mergedata_test.columns.values
                   print j, col_names[j+11],' done'
               print i ,' feature aggregation+merging done'
...
class AnalyzePredict(DataWrangling):
    ...
    def read_aggr_data(self, file, case):
        ...
        if case=='train':
            ...
            return self._traindata,self._trainresponse
        if case=='test':
            self._testdata=data
            print '# rows, columns in test data: ', self._testdata.shape
            print 'columns are: ', self._testdata.columns.values
            #print self._testid
            return self._testid, self._testdata
    ...
    def vowpal_wabbit(self,data):
        import graphlab as gl
        print data.shape,self._trainresponse.shape
        #data['response']=self._trainresponse
        vw_train=gl.SFrame(data=data)
        print vw_train
